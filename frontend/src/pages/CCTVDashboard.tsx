import { Layout } from '@/components/layout/Layout';
import { Video, VideoOff, AlertTriangle, HardHat, Users, Wifi } from 'lucide-react';

const CAMERAS = [
  { id: 'CAM-01', name: 'Zone A — Assembly Line', location: 'Building 1, Floor 1' },
  { id: 'CAM-02', name: 'Zone B — Welding Bay',   location: 'Building 1, Floor 1' },
  { id: 'CAM-03', name: 'Zone C — Paint Shop',    location: 'Building 2, Floor 1' },
  { id: 'CAM-04', name: 'Zone D — Packaging',     location: 'Building 2, Floor 2' },
  { id: 'CAM-05', name: 'Entrance Gate',           location: 'Main Entrance'       },
  { id: 'CAM-06', name: 'Exit Gate',               location: 'Main Exit'           },
];

const StatCard = ({
  label,
  value,
  icon: Icon,
  color,
}: {
  label: string;
  value: string | number;
  icon: React.ElementType;
  color: string;
}) => (
  <div className="glass rounded-xl p-4 flex items-center gap-4">
    <div className={`p-3 rounded-lg ${color}`}>
      <Icon className="w-5 h-5 text-white" />
    </div>
    <div>
      <p className="text-2xl font-bold">{value}</p>
      <p className="text-xs text-muted-foreground">{label}</p>
    </div>
  </div>
);

const NoSignalFeed = ({ cam }: { cam: (typeof CAMERAS)[number] }) => (
  <div className="glass rounded-xl overflow-hidden flex flex-col">
    {/* feed area */}
    <div className="relative bg-black aspect-video flex flex-col items-center justify-center select-none">
      {/* scanlines overlay */}
      <div
        className="absolute inset-0 pointer-events-none opacity-10"
        style={{
          backgroundImage: 'repeating-linear-gradient(0deg, transparent, transparent 2px, rgba(255,255,255,0.3) 2px, rgba(255,255,255,0.3) 4px)',
        }}
      />
      <VideoOff className="w-10 h-10 text-zinc-600 mb-2" />
      <span className="text-zinc-500 text-xs font-mono tracking-widest uppercase">No Signal</span>
      {/* cam id badge */}
      <span className="absolute top-2 left-2 font-mono text-xs text-zinc-500 bg-black/60 px-1.5 py-0.5 rounded">
        {cam.id}
      </span>
      {/* offline badge */}
      <span className="absolute top-2 right-2 flex items-center gap-1 text-xs text-red-400 bg-black/60 px-1.5 py-0.5 rounded">
        <span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse" />
        OFFLINE
      </span>
    </div>

    {/* footer */}
    <div className="px-3 py-2 flex items-center justify-between">
      <div>
        <p className="text-sm font-medium leading-tight">{cam.name}</p>
        <p className="text-xs text-muted-foreground">{cam.location}</p>
      </div>
      <Video className="w-4 h-4 text-muted-foreground shrink-0" />
    </div>
  </div>
);

const CCTVDashboard = () => (
  <Layout>
    <div className="container mx-auto px-6 py-8">
      <div className="mb-8">
        <h1 className="text-4xl font-bold mb-2">CCTV Dashboard</h1>
        <p className="text-muted-foreground">
          Live helmet-compliance monitoring — powered by YOLOv11
        </p>
      </div>

      {/* stats row */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <StatCard label="Total Cameras"    value={CAMERAS.length} icon={Video}          color="bg-blue-600"   />
        <StatCard label="Active Feeds"     value={0}              icon={Wifi}           color="bg-green-600"  />
        <StatCard label="People Detected"  value={0}              icon={Users}          color="bg-violet-600" />
        <StatCard label="Helmet Violations" value={0}             icon={AlertTriangle}  color="bg-red-600"    />
      </div>

      {/* camera grid */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {CAMERAS.map((cam) => (
          <NoSignalFeed key={cam.id} cam={cam} />
        ))}
      </div>

      {/* integration notice */}
      <div className="mt-8 glass rounded-xl p-5 flex items-start gap-3 border border-yellow-500/20">
        <HardHat className="w-5 h-5 text-yellow-400 mt-0.5 shrink-0" />
        <div>
          <p className="text-sm font-medium text-yellow-300 mb-1">Helmet Detection Module Ready</p>
          <p className="text-xs text-muted-foreground">
            Connect your RTSP camera streams or webcam sources in the{' '}
            <span className="text-foreground font-medium">CCTV Testing</span> page, then run the
            YOLOv11 detection module to populate live feeds and compliance stats here.
          </p>
        </div>
      </div>
    </div>
  </Layout>
);

export default CCTVDashboard;
